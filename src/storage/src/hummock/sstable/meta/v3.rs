...
impl MetaPartitionIndex {
    fn validate(&self, meta_offset: u64) -> HummockResult<()> {
        ...
        let _total_block_count = self.shards.iter().try_fold(0u32, |total, shard| {
            total
                .checked_add(shard.block_count)
                .ok_or_else(|| HummockError::decode_error("partitioned meta block count overflow"))
        })?;
    ...
    /// Derive a shard's absolute object offset from the index boundary and lengths.
    ///
    /// V3 stores shard bodies contiguously before the index, so this is runtime state rather
    /// than a second persisted copy of an offset.
    pub(crate) fn shard_offset(&self, meta_offset: u64, shard_idx: usize) -> HummockResult<u64> {
        if shard_idx >= self.shards.len() {
            return Err(HummockError::decode_error(format!(
                "partitioned meta shard index {shard_idx} out of bounds for {} shards",
                self.shards.len()
            )));
        }
        let trailing_shard_bytes =
        ...
        meta_offset.checked_sub(trailing_shard_bytes).ok_or_else(|| {
            HummockError::decode_error(format!(
                "partitioned meta shard bytes {trailing_shard_bytes} exceed index offset {meta_offset}",
            ))
        })
    }
...
pub(crate) fn decode_partitioned_meta_footer_version(buf: &[u8]) -> HummockResult<u32> {
...
mod tests {
    ...
    fn test_partitioned_meta_fixed_wire_fixture() {
        ...
        assert_eq!(index.shard_offset(4096, 0).unwrap(), 3910);
        assert_eq!(index.shard_offset(4096, 1).unwrap(), 4003);
